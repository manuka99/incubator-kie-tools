...
const DiffNestedSection: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <div className="dmn-diff-change-list-v2__nested-section">
    <div className="dmn-diff-change-list-v2__nested-title">{title}</div>
    {children}
  </div>
);
...
const DiffStatRow: React.FC<{ label: string; count: number; color: "green" | "red" | "orange"; suffix?: string }> = ({
  ...
  return (
    <tr className="dmn-diff-change-list-v2__details-row">
      <td className="dmn-diff-change-list-v2__details-cell dmn-diff-change-list-v2__details-cell--label">{label}</td>
      <td className="dmn-diff-change-list-v2__details-cell dmn-diff-change-list-v2__details-cell--value">
        <Label color={color}>
          {count} {suffix}
        </Label>
          ...
          {Object.entries(diff.rules.modified).map(([id, ruleDiff]) => (
            <React.Fragment key={id}>
              ...
              {["inputEntries", "outputEntries", "annotationEntries"].map((entryType) => {
                ...
                return (
                  <tr key={entryType} className="dmn-diff-change-list-v2__details-row">
                    <td className="dmn-diff-change-list-v2__details-cell dmn-diff-change-list-v2__details-cell--label">
  ...
  return (
    <div className="dmn-diff-change-list-v2">
      ...
      <div className="dmn-diff-change-list-v2__content" ref={containerRef}>
        {diffItems.length === 0 ? (
          <div className="dmn-diff-change-list-v2__empty">No changes detected</div>
        ) : (
